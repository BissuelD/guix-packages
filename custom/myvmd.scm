    ...
    (arguments
      `(#:phases
        (modify-phases %standard-phases
          ...
          (add-after 'strip 'verify-pre-validate-runpath
            (lambda* (#:key outputs inputs #:allow-other-keys)
              ; This one is weirdly built (it always points to a subdirectory of the non-exiting "/Projects" )
              ; - removing it altogether 
              (delete-file-recursively (string-append (assoc-ref outputs "out") "/lib/plugins/mafftbins/bins"))
            )
          )
        )
        #:tests? #f))
